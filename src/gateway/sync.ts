...
export async function syncToR2(sandbox: Sandbox, env: MoltbotEnv): Promise<SyncResult> {
  ...
  // Sync: .clawdbot (config), skills, and workspace memory.md
  const syncCmd = `rsync -r --no-times --delete --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/.clawdbot/ ${R2_MOUNT_PATH}/clawdbot/ && rsync -r --no-times --delete /root/clawd/skills/ ${R2_MOUNT_PATH}/skills/ && cp /root/clawd/memory.md ${R2_MOUNT_PATH}/memory.md 2>/dev/null || true && date -Iseconds > ${R2_MOUNT_PATH}/.last-sync`;
